...
export default function SidebarNav({ user }: { user: User }) {
  ...
  return (
    <>
      {/* Mobile top bar */}
      <div className="lg:hidden fixed top-0 left-0 right-0 z-40 bg-[#0e1520] border-b border-[#141d2c] flex items-center justify-between px-4 h-14">
      ...
      {/* Desktop Sidebar */}
      <aside className="hidden lg:flex fixed top-0 left-0 h-full w-[240px] bg-[#0e1520] border-r border-[#141d2c] flex-col z-30">
        {/* User profile */}
        <div className="flex flex-col items-start px-5 pt-7 pb-5">
          ...
          <p className="text-[#7a9ab5] text-[13px] mt-0.5">{user.email}</p>
        ...
        <nav className="flex-1 px-2.5 flex flex-col gap-0.5">
          ...
          {user.role === "ADMIN" && (
            <>
              ...
              <Link
                href="/admin"
                className="flex items-center gap-3.5 px-3 py-2.5 rounded-[8px] text-[14px] text-[#7a9ab5] hover:text-white hover:bg-[#141d2c] transition-colors"
              >
      ...
      {/* Mobile bottom nav */}
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-[#0e1520] border-t border-[#141d2c] flex">
