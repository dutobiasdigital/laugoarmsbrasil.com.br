  ...
  return (
    <div className="max-w-[1160px] py-7">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {STATS.map((stat) => (
          <div key={stat.label} className="bg-[#0e1520] border border-[#141d2c] rounded-[10px] p-5">
            <p className={`font-['Barlow_Condensed'] font-bold text-[40px] leading-none ${stat.color}`}>
              {stat.value}
            </p>
            <p className="text-[#7a9ab5] text-[13px] mt-1">{stat.label}</p>
          </div>
        ))}
      </div>

      {/* Subscription card */}
      <div className="bg-[#0e1520] border border-[#ff1f1f] rounded-xl p-5 lg:p-7 mb-8 relative overflow-hidden">
        ...
        <div className="pl-5 flex flex-col lg:flex-row lg:items-center gap-5">
          <div className="flex-1">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div>
                <p className="text-[#253750] text-[12px] mb-1">Próxima cobrança</p>
                <p className="text-[#d4d4da] text-[15px] font-semibold">
      ...
      <div className="mb-8">
        ...
        <p className="text-[#7a9ab5] text-[14px] mb-5">Acesse rapidamente as últimas edições</p>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
          {recentEditions.map((ed) => (
            <Link
              ...
              className="bg-[#0e1520] border border-[#141d2c] rounded-[8px] overflow-hidden hover:border-zinc-600 transition-colors"
            >
              <div className="h-[148px] bg-[#141d2c] flex items-center justify-center">
                {ed.coverImageUrl ? (
      ...
      {profile.payments.length > 0 && (
        <div>
          ...
          <div className="bg-[#0e1520] border border-[#141d2c] rounded-xl overflow-hidden">
            {/* Header */}
            <div className="bg-[#141d2c] px-5 py-3 grid grid-cols-5 gap-4">
              {["Data", "Plano", "Valor", "Método", "Status"].map((h) => (
                <p key={h} className="text-[#253750] text-[12px] font-semibold tracking-[0.5px]">{h}</p>
              ))}
            </div>
            {/* Rows */}
      ...
      {!sub && (
        <div className="bg-[#0e1520] border border-[#141d2c] rounded-xl p-8 text-center">
          <p className="text-[#7a9ab5] text-[15px] mb-4">Você ainda não tem uma assinatura ativa.</p>
          <Link
            href="/assine"
