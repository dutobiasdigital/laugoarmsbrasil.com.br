...
export default function GuiaQuickAction({ id, status: initial }: { id: string; status: string }) {
  ...
  return (
    <div className={`flex gap-1 items-center ${busy ? "opacity-50 pointer-events-none" : ""}`}>
      {status === "PENDING"   && <button onClick={() => update("ACTIVE")}    className="bg-[#0f381f] hover:bg-[#0f4a25] border border-[#22c55e]/30 text-[#22c55e] text-[11px] h-[26px] px-2.5 rounded-[4px] transition-colors whitespace-nowrap">✓ Aprovar</button>}
