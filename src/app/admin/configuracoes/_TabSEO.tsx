    ...
    <form onSubmit={handleSave} className="flex flex-col gap-6 max-w-[720px]">
      <div>
        ...
        <p className="text-[#526888] text-[13px]">
          Identidade pública do site — meta tags, compartilhamento social e busca orgânica.
        </p>
      </div>
      ...
      <section className="bg-[#0e1520] border border-[#141d2c] rounded-[12px] p-6 flex flex-col gap-4">
        ...
        <div>
          <label className={labelCls}>
            ...
            <span className={`ml-2 font-normal ${descOk ? "text-[#22c55e]" : "text-[#526888]"}`}>
              {descLen}/160{" "}
              {descOk ? "✓ ideal" : descLen < 120 ? "(mínimo 120 chars)" : "(máximo 160 chars)"}
            </span>
          </label>
          <textarea
        ...
        <div>
          ...
          <p className="text-[#526888] text-[11px] mt-1">
            Use termos que o público pesquisa no Google. Separe por vírgula.
          </p>
        </div>
      </section>

      {/* Imagens / Design System */}
      <section className="bg-[#0e1520] border border-[#141d2c] rounded-[12px] p-6 flex flex-col gap-4">
        <div className="flex items-center justify-between pb-2 border-b border-[#141d2c]">
          <h3 className="font-['Barlow_Condensed'] font-bold text-white text-[18px]">
            🖼 Imagens & Design System
          </h3>
          <span className="text-[10px] bg-[#141d2c] text-[#526888] px-2 py-0.5 rounded-full">
        ...
        <div>
          ...
          <input
            value={values["site.logo_url"]}
            onChange={(e) => set("site.logo_url", e.target.value)}
            placeholder="https://..."
            type="url"
            className={inputCls}
          />
          <p className="text-[#526888] text-[11px] mt-1">
            PNG transparente recomendado. Exibido no header do site.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <input
              value={values["site.favicon_url"]}
              onChange={(e) => set("site.favicon_url", e.target.value)}
          ...
          <div>
            ...
            <input
              value={values["site.og_image_url"]}
              onChange={(e) => set("site.og_image_url", e.target.value)}
